...
$sql2_query="SELECT * FROM `product` WHERE uid='$id'";//資料庫語法 這裡要改成 超過日期就不要顯示在上面
    if($stmt=$connect->query($sql2_query)){//將名字為conn的資料庫連結檔 傳到query($sql2_query)去執行SQL語法  將結果命名為$stmt
        ...
        echo "<tr><td  colspan='14' align='center'>".$id."的接案清單</td></tr>";
        echo "<tr>";
        ...
        echo "<td align='center'>訂單編號</td>
              ...
              <td align='center' style='display:none'>接案</td>";
        ...
        while($row=mysqli_fetch_assoc($stmt)){//mysqli_fetch_object($stmt)是回傳當下的結果值($stmt)  並另外存為result
            ...
            echo "<td align='center' style='display:none'>".$row['bartender_id']."</td>";
            ...
            echo "<td align='center' style='display:none'><a href=advanced_userCaseUpdate.php?id=".$row["caseNo"].">我要接案</a></td>";
        }    
