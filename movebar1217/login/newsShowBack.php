...
<br>
<a href="news_Send.php">發佈消息</a>
<?php

  
        
        $sql2_query="SELECT * FROM `news` ";//資料庫語法 這裡要改成 超過日期就不要顯示在上面
        if($stmt=$connect->query($sql2_query)){//將名字為conn的資料庫連結檔 傳到query($sql2_query)去執行SQL語法  將結果命名為$stmt
            ...
            echo "<tr>
                    <td  colspan='5' align='center'>"."消息</td>
            ...
            echo "<td align='center'>種類</td>
                  <td align='center'width=150 >標題</td>
                  <td align='center'width=150 >發布日期</td>
                  <td align='center'width=150 >結束日期</td>
                  <td align='center'width=150 >功能</td>";
            ...
            while($row=mysqli_fetch_assoc($stmt)){//mysqli_fetch_object($stmt)是回傳當下的結果值($stmt)  並另外存為result
                ...
                echo "<td align='center'><a href=news_delete.php?id=".$row["news_number"].">刪除</a></td>";
